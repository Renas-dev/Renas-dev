## Hey, I'm Renas 

Junior Full Stack Developer who enjoys building backend systems, APIs and shipping things that actually run in production.

I like understanding how things work **end-to-end** — from API design to containers, pipelines and deployment.

---

### Main stack
- Java (Spring Boot)
- C# (ASP.NET Core)
- REST APIs
- SQL (PostgreSQL, MySQL)

---

### DevOps & infrastructure

![CI](https://img.shields.io/badge/CI-GitHub_Actions-success)
![CD](https://img.shields.io/badge/CD-VPS_Deploy-success)

- Docker
- CI/CD pipelines (GitHub Actions)
- Separate CI and CD runners
- Automated build, test and deploy
- Self-hosted VPS deployments
- Kubernetes (fundamentals)

---

### Other experience
- PHP (Laravel)
- Python
- HTML & CSS
- Flutter

---

### Projects
- **EepyTimes** — sleep cycle calculator  
  https://www.eepytimes.com  
  Private backend with automated CI/CD and VPS deployment

- **ASP.NET Core REST API Fundamentals**  
  Incremental REST API project (local CRUD → Docker → JWT authentication)

- **C# Fundamentals**  
  Core language concepts and algorithm implementations  
  (Binary Search, BFS, DFS, Sliding Window)

---

### Focus areas
- Clean backend architecture
- API design
- Containerized development
- Reliable CI/CD pipelines
